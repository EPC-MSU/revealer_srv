...
* Решение: доработать скрипт для вашей операционной системы (`netset.sh` или `netset.bat` в папке `scripts`), чтобы переданные аргументы, описанные в разделе "Разъяснения к полям конфигурационного файла", использовались для изменения настроек устройства и указать путь до вашего скрипта в конфигурационного файла `config.ini`, а также задать пароль для смены настроек. Поле пароля можно оставить пустым, если требуется. Например, если `netset.sh` находится в папке `scripts` и пароль хочется установить `0000`:
```
password = 0000
